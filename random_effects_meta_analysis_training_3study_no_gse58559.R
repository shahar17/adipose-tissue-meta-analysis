...
output_dir <- file.path(project_dir, "training_3study_no_gse58559_meta_output")
...
included_comparisons <- data.frame(
  study = c("GSE116801", "GSE208032", "GSE43471"),
  comparison = c(
...
excluded_comparisons <- data.frame(
  ...
  inclusion_role = "excluded_mixed_training_diet_intervention",
  note = "Excluded for this sensitivity analysis because the intervention combines exercise training with diet/energy deficit, and local metadata does not identify diet-only versus exercise-plus-diet samples.",
  stringsAsFactors = FALSE
)
...
fit_gene <- function(gene_df) {
  ...
  if (inherits(fit, "error")) {
    return(data.frame(
      gene = gene_df$gene[[1]],
      n_studies = n_studies,
      all_3_studies = n_studies == n_total_studies,
...
if (length(gene_results) == 0) {
  stop("No genes were represented in at least two included studies.")
}
...
write.csv(
  per_cohort,
  file.path(output_dir, "per_study_differential_expression_training_3study_no_gse58559.csv"),
  row.names = FALSE
)

write.csv(
  meta_results,
  file.path(output_dir, "training_3study_no_gse58559_meta_results.csv"),
  row.names = FALSE
)

write.csv(
  meta_results[meta_results$all_3_studies, ],
  file.path(output_dir, "training_3study_no_gse58559_meta_results_all_3_studies.csv"),
  row.names = FALSE
)
...
summary_table <- data.frame(
  metric = c(
    ...
    "genes_meta_analyzed_all_3_studies",
    "fit_errors"
  ),
  value = c(
    nrow(per_cohort),
    ...
    nrow(meta_results),
    sum(meta_results$all_3_studies),
...
write.csv(
  summary_table,
  file.path(output_dir, "training_3study_no_gse58559_meta_summary.csv"),
  row.names = FALSE
)

message(
  "Wrote 3-study training meta-analysis results to ",
  file.path(output_dir, "training_3study_no_gse58559_meta_results.csv")
)
print(summary_table)
